...
public class GeneralContainerViewFactory {
    ...
    @Autowired
    public GeneralContainerViewFactory(Set<SpecificContainerFactory> services, ContainerServices cam) {
    ...
    private Container createFromFactory(ContainerView view) {
        ...
        Container parent =
            cam.fetchContainerByName(view.getParent()).orElseThrow(() -> new RuntimeException("Parent not found"));
    ...
    private SpecificContainerFactory<? extends Container> getFactory(ContainerView view) {
        return StreamSupport.stream(services.spliterator(), false)
            ...
            .orElseThrow(() -> new IllegalArgumentException("Unknown Type to construct " + view.getType()));
    }

    private SpecificContainerFactory<? extends Container> getFactory(AbstractReferencable container) {
        return services.stream()
            .filter((factory) -> factory.getContainerClass().isAssignableFrom(container.getClass()))
            .findFirst()
            .orElseThrow(() -> new IllegalArgumentException("Unknown Type to construct " + container.getClass()));
    }
